...
from train_cnn_pytorch import Config as CNNConfig, create_model as create_cnn_model, create_mlp_model
...
def load_models(device: torch.device) -> Dict[str, Tuple[torch.nn.Module, str]]:
    ...
    mlp = create_mlp_model(dropout=CNNConfig.DROPOUT, device=device)
    if os.path.exists(CNNConfig.MLP_MODEL_SAVE_PATH):
        state = torch.load(CNNConfig.MLP_MODEL_SAVE_PATH, map_location=device)
        mlp.load_state_dict(state)
    else:
        print(f"[警告] 未找到权重文件: {CNNConfig.MLP_MODEL_SAVE_PATH}，将使用随机初始化权重。")
    mlp.eval()
    models["mlp"] = (mlp, CNNConfig.MLP_MODEL_SAVE_PATH)
